...
> 대규모 APM 지표까지는 남기지 못했지만, 외부 의존성(Embedding/Rerank/LLM) 구간의 **지연/실패 패턴을 로그로 관찰**하고,
> 병목이 되는 단계와 실패 시나리오를 기준으로 **호출 범위·대상·fallback 전략**을 조정했습니다.  
> 자세한 내용은 Wiki의 Decision Log에 정리했습니다.

### 1) 일반 검색에서 리랭킹 제거
- **문제**: 일반 검색에 하이브리드 + 리랭킹을 적용하니 응답 지연이 커져 사용자 체감이 나빠짐  
- **결정**: 일반 검색은 리랭킹을 제거하고, 리랭킹은 AI 검색에만 적용  
- **이유/결과**: 검색 품질 체감은 크게 떨어지지 않으면서, “느려서 답답한” 케이스를 줄여 핵심 경로를 안정화

### 2) 캐시 전략 재정의: 일반 검색 제외, AI 검색만 캐싱
- **문제**: 일반 검색은 리랭킹 제거 이후 충분히 빠른데, 캐시 도입의 이점이 제한적  
- **결정**: 일반 검색 캐싱 제외 / AI 검색만 캐싱 적용  
- **이유/결과**: AI 검색은 rerank/LLM 호출로 지연이 커서 캐싱 가치가 높았고, “필요한 곳”에만 적용해 복잡도를 통제

### 3) AI 검색 캐시 키를 ‘의미 기반’으로 설계 (Komoran + 정렬)
- **문제**: 단순 문자열 캐시는 문장/띄어쓰기/단어 순서가 달라지면 hit가 어려움  
- **결정**: Komoran으로 핵심 키워드만 추출하고 가나다순으로 정렬해 캐시 키 생성  
- **이유/결과**: 같은 의미의 질문이 형태만 달라도 캐시 적중 가능성을 확보하면서, AI 검색 응답을 더 안정적으로 유지

### 4) 외부 API 실패 시 검색 결과 공백 방지(fallback)
- **문제**: 외부 API 오류가 발생하면 결과 리스트가 비는 케이스가 발생  
- **결정**: 각 단계에서 실패하면 “API 호출 직전 후보 리스트”를 fallback으로 사용  
- **이유/결과**: 품질이 일부 저 되더라도 최소 검색 결과는 항상 제공하도록 만들어 사용자 경험을 안정화

### 5) 리랭킹 입력 Top-N 제한(약 50 → 10)
- **문제**: 하이브리드 후보를 그대로 리랭킹에 보내면 payload가 커져 지연이 커짐  
- **결정**: 리랭킹 대상 도서를 Top 10권으로 제한  
- **이유/결과**: 체감 품질은 유지되면서, 길게 늘어지는 지연 케이스가 줄어 응답 안정성이 좋아짐

### 6) Gemini 호출 축소(Top 5 + 200자 제한)
- **문제**: 많은 책에 추천이유를 생성하려다 프롬프트가 커져 429/timeout이 잦아짐  
- **결정**: 상위 5권만 요청 + 응답 길이 약 200자 제한  
- **이유/결과**: 사용자 체감이 큰 구간에 집중하면서도 실패/지연 빈도를 줄여 AI 기능을 안정화
